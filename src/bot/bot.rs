...
impl Bot {
    ...
    fn generate_placements(&mut self, frame: &Frame, depth_one: bool) -> HashSet<Frame> {
        ...
        for premove in premoves {
            if let Some(premoved_frame) = frame.try_commands(&premove) {
                self.flood_in_direction(
                    premoved_frame.clone(),
                    MoveLeft,
                    premove.clone(),
                    &mut possible_placements,
                    depth_one,
                );
                self.flood_in_direction(
                    premoved_frame.clone(),
    ...
    fn generate_placements2(
        &mut self,
        frame: &Frame,
        depth_one: bool,
        commands: Vec<Command>,
        seen: &mut HashSet<Frame>,
        toggle: usize,
    ) -> HashSet<Frame> {
        let mut possible_placements = HashSet::new();
        ...
        for move_dir in moves[toggle % 2] {
            if let Some(new_frame) = frame.try_command(&move_dir) {
                if !seen.contains(&new_frame) {
                    ...
                    possible_placements.extend(self.generate_placements2(
                        &new_frame,
                        depth_one,
                        commands2.clone(),
                        seen,
    ...
    fn add_if_unseen(
        ...
        depth_one: bool,
        commands: Vec<Command>,
        possible_placements: &mut HashSet<Frame>,
    ) -> bool {
    ...
    fn flood_in_direction(
        &mut self,
        promoved_frame: Frame,
        move_dir: Command,
        mut premoves: Vec<Command>,
        possible_placements: &mut HashSet<Frame>,
        depth_one: bool,
    ) {
    ...
    fn filter_top_n_placements(&self, placements: HashSet<Frame>, n: usize) -> Vec<Frame> {
        if n == 0 {
            // don't filter
            return placements.iter().cloned().collect();
        }

        let mut sorted_placements = placements
            .iter()
            .map(|frame| {
                let score = Evaluator::new(frame.clone()).eval(false);
                (frame.clone(), score)
